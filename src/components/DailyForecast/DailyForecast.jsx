    ...
    <section className={`daily-forecast-container ${theme}`}>
      {forecast.map((day) => (
        <div key={day.date} className="daily-forecast-card">
          <p>{day.day}</p>
          <p><span className={`daily-min-temp_symbol ${theme}`}>↓</span> {Math.round(day.minTemp)} {temperatureUnit}</p>
          <p><span className={`daily-max-temp_symbol ${theme}`}>↑</span> {Math.round(day.maxTemp)} {temperatureUnit}</p>
